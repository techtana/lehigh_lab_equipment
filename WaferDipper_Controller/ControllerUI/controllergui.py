...
from SlackComm import messenger
...
class ProgButtons(tk.Frame):
    def __init__(self, master, pgrm, cnvas, motor, ser, slack_url):
        tk.Frame.__init__(self, master)
    ...
    def pgrm_run(self):
        ...
        for k in range(1, avail_entry(self.pgrm.entriesX)):
            ...
            statement = "<Xabs:" + str(int(float(self.parse_mm_to_step(1, self.pgrm.entriesX[k].get())))) + \
                        ...
                        ",T:" + str(int(float(self.pgrm.entriesWait[k].get()))) + ">"
            print(statement)
            self.ser.write(str.encode(statement))
        ...
        self.motor.origin()
        messenger.send_message_to_slack(self.slack_url,
                                        'Dip Coater has completed a run.' +
